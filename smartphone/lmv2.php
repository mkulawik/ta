<link rel="stylesheet" type="text/css" media="all" href="smartphone/css/ratetable.css" />
...
<div class="ratetable-button-container"><a <? if(isset($_SESSION['idm'])){?> href="write_review_movie.php?idm=<? echo $movid[$j];?>"  <?} else {?> href="log-in.php" <? }?>>Rate me</a></div>     
              </div>
    </div>
...
?>
	...
	<div> <a href="movie.php?idm=<? echo $movid[$j];?>"> <img src="movies/<? echo $movimg[$j];?>" width="45"/></a> </div>
          <div class="ratetable-header-inner">
            <div class="ratetable-fld-rate"> <span class="lmov2-cat"><<?=getRatingColor ($movscore[$j]);?>><?  echo $movscore[$j]; ?> </<?=getRatingColor ($movscore[$j]);?>></span></div>
	    <p>By: <a href="member_profile.php?idm=<? echo $movmemid[$j];?>"><? echo $movmem[$j];?></a> </p>   
                   </div>
        </div>
        <ul class="features">
